...
Param(
    [array]$ServersToQuery = (Get-WmiObject -Class Win32_ComputerSystem -Property Name).Name,
    ...
    $FilePath = "$env:temp\$Date`_RDP_Report.csv"
...
echo "--------------------------------------"
echo "This is a report of who has RDP'd" 
echo "to this server"
echo "--------------------------------------"
cat $FilePath
echo "--------------------------------------"
